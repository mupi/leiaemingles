$(document).ready(function(){
	...
	$.get('xml/parts'+dominoType+'.xml', function(xml){
...
function transform(droppable, draggable, actualPart){
	...
	if(holeLast == 1){
		//Muda suas dimensões
		$(draggable).css('background', 'url(imagens/dominov.gif")');
...
function deTransform(droppable, draggable, actualPart){
	...
	if($(droppable).attr('class').search('holeLast') != -1){
		//Muda suas dimensões
		$(draggable).css('background', 'url("imagens/dominoh.gif")');
...
function popUp(phrase){
	...
	if(phrase == null){
		$('#message').text('Ops! Esta combina\u00e7\u00e3o vai bloquear as próximas jogadas. Tente outra!');
		$('#message').css('margin-top', '40px');
		$('#message').css('color', 'black');
		$('#close').css('display', 'none');
		$('#popUp').css('background', "url('imagens/fundo_prompt.gif') repeat");
